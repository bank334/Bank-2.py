...
staff = ["u801fe367c48e4fe31ab0270933a0616f"]
adminMID = ["u801fe367c48e4fe31ab0270933a0616f"]
...
# ----------------- NOTIFED MEMBER JOIN GROUP
        if op.type == 17:
            ...
            cb = Message()
            cb.to = op.param1
            cb.text = cl.getContact(op.param2).displayName + "\n เข้าร่วมกลุ่ม  " + group.name + "\n💖ยินดีต้อนรับจ้า💖"
            cl.sendMessage(cb)
#------------------ NOTIFED MEMBER JOIN GROUP
        if op.type == 19:
            if op.param2 in Bots:
                return
            cl.sendText(op.param1,cl.getContact(op.param2).displayName + "\n ได้ลบคุณออกจากกลุ่ม"" + group.name + "\n  \n⚠แล้ว⚠")
